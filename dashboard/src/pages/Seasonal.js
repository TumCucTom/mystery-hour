...
function renderPage(container, seasonal) {
  const SEASON_EMOJI = {
  ...
  container.innerHTML = `
    <div class="page-header">
      ...
      <p>Do Christmas episodes have different topics? What happened during COVID?</p>
    </div>

    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;margin-bottom:24px">
      ${Object.entries(seasonal).filter(([k]) => k !== 'covid').map(([season, data]) => {
        const emoji = SEASON_EMOJI[season] || '📆'
        const desc  = SEASON_DESC[season] || season
        const topClusters = (data.top_clusters || []).map(c => `Cluster ${c.id}`).join(', ')
        return `
          <div class="card">
            ...
            <div style="margin-top:12px;font-size:13px">
              <div style="color:var(--color-muted);margin-bottom:4px">Top clusters:</div>
            ...
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--color-border)">
              ...
              <div style="flex:1">
                ${(data.top_clusters || []).map(c => `
                  <span style="display:inline-block;background:var(--color-primary);color:white;padding:3px 8px;border-radius:4px;font-size:12px;margin:2px">Cluster ${c.id} (${(c.frac*100).toFixed(1)}%)</span>
                `).join('')}
              </div>
            </div>
          `
